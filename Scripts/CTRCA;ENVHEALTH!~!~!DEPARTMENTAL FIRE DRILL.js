...
try {
  ...
  if(!matches(reportName,null,undefined,"")){
    ...
    // declare report with global scope 
    var myResults = myRunReportAttach(parentCapId,reportName,rptParams);
    ...
    if (myResults.getSuccess()) {
      ...
      reportModel = getRecordModelByDocumentName(parentCapId, reportName)
      if (reportModel) {
 ...
 * @returns {reportInfoScript object}
