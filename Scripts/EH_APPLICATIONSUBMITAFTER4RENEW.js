...
function copyKeyInfo(srcCapId, targetCapId)
{
	...
	//Copy Cap Basic
	copyCapBasicInfo(srcCapId, targetCapId)
	
	
	
}

function copyCapBasicInfo(srcCapId,targetCapId)
{
	if(srcCapId != null && targetCapId != null)
	{
		var srcCapIdResultObj = aa.cap.getCapByPK(srcCapId,true);
		var targetCapIdResultObj = aa.cap.getCapByPK(targetCapId,true);
		if(srcCapIdResultObj.getSuccess() && targetCapIdResultObj.getSuccess() )
		{
			var srcCapIdResult = srcCapIdResultObj.getOutput();
			var targetCapIdResult = targetCapIdResultObj.getOutput();

			targetCapIdResult.setSpecialText(srcCapIdResult.getSpecialText());
			aa.cap.editCapByPK(targetCapIdResult);
		}
		
		var srcCapWorkResultObj = aa.cap.getCapWorkDesByPK(srcCapId);
		var targetCapWorkResultObj = aa.cap.getCapWorkDesByPK(targetCapId);
		if(srcCapWorkResultObj.getSuccess() && targetCapWorkResultObj.getSuccess() )
		{
			var srcCapWork = srcCapWorkResultObj.getOutput();
			var targetCapWork = targetCapWorkResultObj.getOutput();

			targetCapWork.setDescription(srcCapWork.getDescription());
			aa.cap.editCapWorkDes(targetCapWork.getCapWorkDesModel());
		}
	}
...
function copyContactsWithAddress(pFromCapId, pToCapId)
{
   ...
   if (capContactResult.getSuccess())
   {
      ...
      for (yy in Contacts)
      {
		 ...
		 if((newContact.getContactType() +"") == "Permit Holder")
		 {
			
			 newContact.setContactType("Applicant");	
		 }
