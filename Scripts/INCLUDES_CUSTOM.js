...
function updateCapAldID()
{
	...
	aa.debug("parentCapId=",parentCapId);
	
	newAltId= getNewAltID(parentCapId);
	aa.debug("newAltId=",newAltId);
	if(newAltId)
	{
		var result = aa.cap.updateCapAltID(capId,newAltId).getOutput();
		if(result)
		{	
			logDebug("Update Alt ID Succesfully.");
		}
		else
		{
...
function getNewAltID(parentLicenseCAPID)
{
	...
	if(childList)
	{
		...
		for(x in childList)
		{
			...
			logDebug(tmpString1.indexOf(year));
	...
	return capAldIdArray[0]+"-"+capAldIdArray[1]+"-"+capAldIdArray[2]+"-"+count;	
}
 ...
 * 1. the documentation for the parameters indicated hashTable instead of hashmap
 * 2. when determining if arguments[2] was a string, function.getClass() functions which is not supported by strings.
 * @example
 *		runReportAttach(capId,"ReportName","altid",capId.getCustomID(),"months","12");
 *		runReportAttach(capId,"ReportName",paramHashmap);
 * @param capId
...
function runReportAttach(itemCapId,aaReportName)
	...
	if(permit.getOutput().booleanValue()) 
        ...
        logDebug("Report " + aaReportName + " for " + itemCapId.getCustomID() + " failed with error: " + reportResult.getErrorMessage() );
        return false;
      }
		}else{
